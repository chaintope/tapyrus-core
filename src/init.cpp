 ...
 * between tapyrusd, and tapyrus-qt and non-server tools.
*/
...
void SetupServerArgs()
{
    ...
    // Hidden Options
    std::vector<std::string> hidden_args = {"-rpcssl", "-benchmark", "-h", "-help", "-socks", "-tor", "-debugnet", "-whitelistalwaysrelay", "-promiscuousmempoolflags", "-blockminsize", "-dbcrashratio", "-forcecompactdb", "-usehd",
        // GUI args. These will be overwritten by SetupUIArgs for the GUI
         "-choosedatadir", "-lang=<lang>", "-min", "-resetguisettings", "-rootcertificates=<file>", "-splash", "-uiplatform"};
...
#if USE_UPNP
    gArgs.AddArg("-upnp", strprintf("Use UPnP to map the listening port (default: %u)", DEFAULT_UPNP), false, OptionsCategory::CONNECTION);
#else
    ...
    gArgs.AddArg("-datacarriermultiple", strprintf("Allow more than one OP_RETURN and data in a data carrier transactions. (default: false)"), false, OptionsCategory::NODE_RELAY);
    ...
    gArgs.AddArg("-blockfeatures=<n>", "Override block features to test forking scenarios", true, OptionsCategory::BLOCK_CREATION);
...
{
    std::string strCopyrightHolderBitcoin = strprintf("Copyright (C) %i-%i %s", 2009, COPYRIGHT_YEAR, COPYRIGHT_HOLDERS_FINAL);
    std::string strCopyrightHolderChaintope = strprintf("Copyright (C) %i-%i %s", COPYRIGHT_YEAR, COPYRIGHT_YEAR_CHAINTOPE, COPYRIGHT_HOLDER_CHAINTOPE);
...
bool AppInitParameterInteraction()
{
    ...
    hashAssumeValid = uint256S(gArgs.GetArg("-assumevalid", ""));
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if tapyrus is started in the future "
        ...
        do {
            ...
            try {
                ...
                if (!fReset) {
                    ...
                    // It reduces validity/have-data flags in mapBlockIndex and rebuilds
                    // setBlockIndexCandidates for the current chainActive.
