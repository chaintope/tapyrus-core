...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " tapyrus-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  tapyrus-tx [options] <hex-tx> [commands]  Update hex-encoded tapyrus transaction\n" +
            "or:     tapyrus-tx [options] -create [commands]   Create hex-encoded tapyrus transaction\n" +
            "\n";
...
{
    int64_t newFeatures;
    if (!ParseInt64(cmdVal, &newFeatures) || newFeatures < 1 || newFeatures > CTransaction::MAX_STANDARD_FEATURES)
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded tapyrus transaction
