    ...
    transactionView(0), abandonAction(0), bumpFeeAction(0), columnResizingFixer(0)
{
    ...
    // Connect actions
    // Note: Qt 6 QSignalMapper no longer provides mapped() signals.
    // Third party URL connections are made directly in contextualMenu() using lambdas
    ...
    // Delay timer for amount changes
    connect(amountWidget, &QLineEdit::textChanged, amount_typing_delay, [amount_typing_delay](){ amount_typing_delay->start(); });
    connect(amount_typing_delay, &QTimer::timeout, this, &TransactionView::changedAmount);
    // Delay timer for search changes
    connect(search_widget, &QLineEdit::textChanged, prefix_typing_delay, [prefix_typing_delay](){ prefix_typing_delay->start(); });
        ...
        if (_model->getOptionsModel())
        {
            ...
            for (int i = 0; i < listUrls.size(); ++i)
            {
                ...
                if (!host.isEmpty())
                {
                    ...
                    connect(thirdPartyTxUrlAction, &QAction::triggered, this, [this, thirdPartyTxUrlAction](){ mapperThirdPartyTxUrls->map(thirdPartyTxUrlAction); });
...
void TransactionView::changedAmount()
{
    ...
    if (TapyrusUnits::parse(model->getOptionsModel()->getDisplayUnit(), amountWidget->text(), &amount_parsed)) {
...
void TransactionView::exportClicked()
{
    ...
    writer.addColumn(TapyrusUnits::getAmountColumnTitle(model->getOptionsModel()->getDisplayUnit()), 0, TransactionTableModel::FormattedAmountRole);
...
QWidget *TransactionView::createDateRangeWidget()
{
    ...
    // Notify on change
    connect(dateFrom, &QDateEdit::dateChanged, this, &TransactionView::dateRangeChanged);
    connect(dateTo, &QDateEdit::dateChanged, this, &TransactionView::dateRangeChanged);
