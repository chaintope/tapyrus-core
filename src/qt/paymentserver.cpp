...
const char* BIP71_MIMETYPE_PAYMENT = "application/tapyrus-payment";
const char* BIP71_MIMETYPE_PAYMENTACK = "application/tapyrus-paymentack";
const char* BIP71_MIMETYPE_PAYMENTREQUEST = "application/tapyrus-paymentrequest";
...
void PaymentServer::initNetManager()
{
    ...
    // netManager is used to fetch paymentrequests given in tapyrus: URIs
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    for (const std::pair<CScript, CAmount>& sendingTo : sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            // Unauthenticated payment requests to custom tapyrus addresses are not supported
