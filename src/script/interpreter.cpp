 ...
 * 
 * Parameter dataSignature added for use with OP_CHECKDATASIG
 * In signatures that are input of OP_CHECKDATASIG 
 * there is no <hashtype> byte
 */
bool static IsValidSignatureEncoding(const std::vector<unsigned char> &sig, bool dataSignature = false) {
    ...
    const unsigned int numparts = dataSignature ? 8 : 9;
    const unsigned int size = dataSignature ? 72 : 73;

    // Minimum and maximum size constraints.
    if (sig.size() < numparts) return false;
    if (sig.size() > size) return false;

    // A signature is of type 0x30 (compound).
    if (sig[0] != 0x30) return false;

    // Make sure the length covers the entire signature.
    if (sig[1] != sig.size() - (dataSignature ? 2 : 3)) return false;
    ...
    // of the elements.
    if ((size_t)(lenR + lenS +  (dataSignature ? 6 : 7)) != sig.size()) return false;
...
bool EvalScript(std::vector<std::vector<unsigned char> >& stack, const CScript& script, unsigned int flags, const BaseSignatureChecker& checker, ColorIdentifier *colorId,  ScriptError* serror)
{
        ...
        while (pc < pend)
        {
            ...
            switch (opcode)
            {
                ...
                case OP_CHECKSIGVERIFY:
                {
                    ...
                    // Drop the signature from the script code before hashing
...
template <class T>
bool GenericTransactionSignatureChecker<T>::CheckSig(const std::vector<unsigned char>& vchSigIn, const std::vector<unsigned char>& vchPubKey, const CScript& scriptCode) const
{
...
template uint256 SignatureHash(const CScript& scriptCode, const CTransaction& txTo, unsigned int nIn, int nHashType, const CAmount& amount);
template uint256 SignatureHash(const CScript& scriptCode, const CMutableTransaction& txTo, unsigned int nIn, int nHashType, const CAmount& amount);
template class GenericTransactionSignatureChecker<CTransaction>;
template class GenericTransactionSignatureChecker<CMutableTransaction>;

bool VerifyScript(const CScript& scriptSig, const CScript& scriptPubKey, unsigned int flags, const BaseSignatureChecker& checker, ColorIdentifier& colorId, ScriptError* serror)
{
    set_error(serror, SCRIPT_ERR_UNKNOWN_ERROR);
    ...
    if (!EvalScript(stack, scriptSig, flags, checker, &colorId, serror))
        // serror is set
        return false;
    stackCopy = stack;
    if (!EvalScript(stack, scriptPubKey, flags, checker, &colorId, serror))
        // serror is set
        return false;
    if (stack.empty())
        ...
        (scriptPubKey.IsColoredPayToScriptHash() && GetSoftForkManager().IsEnabled(SCRIPT_VERIFY_CP2SH_COLORED, flags)))
    {
        ...
        if (!EvalScript(stack, pubKey2, flags, checker, nullptr, serror))
            // serror is set
            return false;
        if (stack.empty())
            return set_error(serror, SCRIPT_ERR_EVAL_FALSE);
