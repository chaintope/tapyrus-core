 ...
 * where R and S are not negative (their first byte has its highest bit not set), 
 * and not excessively padded (do not start with a 0 byte, unless an otherwise negative number follows, 
...
bool CheckDataSignatureEncoding(const valtype &vchSig, uint32_t flags, ScriptError *serror) {
    ...
    if ((flags & (SCRIPT_VERIFY_DERSIG | SCRIPT_VERIFY_LOW_S |
        ...
        !IsValidDataSignatureEncoding(vchSig)) {
