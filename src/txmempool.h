...
private:
    ...
    int32_t sigOpCost;         //!< Total sigop cost
...
public:
    ...
    std::vector<std::pair<uint256, txiter> > vTxHashes; //!< All tx hashmalfix hashes/entries in mapTx, in random order
...
public:
     ...
     */
    bool CalculateMemPoolAncestors(const CTxMemPoolEntry& entry, setEntries& setAncestors, int64_t limitAncestorCount, int64_t limitAncestorSize, int64_t limitDescendantCount, int64_t limitDescendantSize, std::string& errString, bool fSearchForParents = true) const EXCLUSIVE_LOCKS_REQUIRED(cs);
