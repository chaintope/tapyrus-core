...
bool CNode::ReceiveMsgBytes(const char *pch, unsigned int nBytes, bool& complete)
{
    ...
    while (nBytes > 0) {
        ...
        if (vRecvMsg.empty() ||
            ...
            vRecvMsg.push_back(CNetMessage(FederationParams().MessageStart(), SER_NETWORK, INIT_PROTO_VERSION));
...
static void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        std::string strDesc = "Tapyrus " + FormatFullVersion();
...
void CConnman::ThreadDNSAddressSeed()
{
    ...
    for (const std::string &seed : vSeeds) {
        ...
        if (HaveNameProxy()) {
            ...
            std::string host = strprintf("x%x.i%s.%s", requiredServiceBits, FederationParams().NetworkIDString(), seed);
...
bool CConnman::OutboundTargetReached(bool historicalBlockServingLimit)
{
    ...
    if (historicalBlockServingLimit)
    {
        ...
        uint64_t buffer = timeLeftInCycle / 600 * CURRENT_MAXBLOCKSIZE();
...
void CConnman::PushMessage(CNode* pnode, CSerializedNetMsg&& msg)
{
    ...
    CMessageHeader hdr(FederationParams().MessageStart(), msg.command.c_str(), nMessageSize);
