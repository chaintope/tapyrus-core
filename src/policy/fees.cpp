...
void CBlockPolicyEstimator::processTransaction(const CTxMemPoolEntry& entry, bool validFeeEstimate)
{
    ...
    // Feerates are stored and reported as TPC-per-kb:
...
bool CBlockPolicyEstimator::Write(CAutoFile& fileout) const
{
    try {
        LOCK(cs_feeEstimator);
        fileout << ESTIMATOR_FILE_VERSION; // version required to read: 0.14.99 or later
...
bool CBlockPolicyEstimator::Read(CAutoFile& filein)
{
    try {
        ...
        if (nVersionRequired < ESTIMATOR_FILE_VERSION) {
            LogPrintf("%s: incompatible old fee estimation data (non-fatal). Version: %d\n", __func__, nVersionRequired);
        } else { // New format introduced in bitcoin in 149900
