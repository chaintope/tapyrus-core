...
{
    BOOST_CHECK_EQUAL(script.GetSigOpCount(false), inaccurate_sigops + datasigops);
    BOOST_CHECK_EQUAL(script.GetSigOpCount(true), accurate_sigops + datasigops);

    const CScript p2sh = GetScriptForDestination(CScriptID(script));
    const CScript scriptSig = CScript() << OP_0 << Serialize(script);

    BOOST_CHECK_EQUAL(p2sh.GetSigOpCount(scriptSig), accurate_sigops + datasigops);

    // Check that GetSigOpCount do not report sigops in the P2SH script when the
    // P2SH flags isn't passed in.
    BOOST_CHECK_EQUAL(p2sh.GetSigOpCount(scriptSig), 0U);
    ...
    BOOST_CHECK_EQUAL(script.GetSigOpCount(p2sh),
...
BOOST_AUTO_TEST_CASE(GetSigOpCount)
{
    ...
    scriptSig2 << OP_1 << ToByteVector(dummy) << ToByteVector(dummy)
               << Serialize(s3);
    BOOST_CHECK_EQUAL(
        p2sh.GetSigOpCount(scriptSig2), 3U);
    BOOST_CHECK_EQUAL(
        p2sh.GetSigOpCount(scriptSig2), 3U);
    BOOST_CHECK_EQUAL(p2sh.GetSigOpCount(scriptSig2), 0U);
    ...
    int flags)
{
    ...
    bool ret = VerifyScript(
        inputi.vin[0].scriptSig, output.vout[0].scriptPubKey, nullptr, flags,
...
BOOST_AUTO_TEST_CASE(GetTxSigOpCost)
{
    ...
    // Multisig script (legacy counting)
    {
        ...
        // signature operation.
        BOOST_CHECK_EQUAL(
            GetTransactionSigOpCost(CTransaction(spendingTx), coins, flags),
            0);
        // creationTx contains two signature operations in its scriptPubKey, but
        // legacy counting is not accurate.
        BOOST_CHECK_EQUAL(
            GetTransactionSigOpCost(CTransaction(creationTx), coins, flags),
        ...
        // not passed in.
        BOOST_CHECK_EQUAL(GetTransactionSigOpCost(CTransaction(spendingTx),
                              coins, SCRIPT_VERIFY_NONE),
            0);
        BOOST_CHECK_EQUAL(GetTransactionSigOpCost(CTransaction(creationTx),
                              coins, SCRIPT_VERIFY_NONE),
            MAX_PUBKEYS_PER_MULTISIG);
    }
...
BOOST_AUTO_TEST_CASE(test_max_sigops_per_tx)
{
    ...
    for (size_t i = 0; i < MAX_BLOCK_SIGOPS_COST; i++) {
