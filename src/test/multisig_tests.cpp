...
BOOST_AUTO_TEST_CASE(multisig_verify)
{
    ...
    for(TestMode mode : {TestMode::ECDSA, TestMode::SCHNORR, TestMode::MIXED})
    {
        ...
        BOOST_CHECK_EQUAL(VerifyScript(s, a_and_b, nullptr, flags, MutableTransactionSignatureChecker(&txTo[0], 0, amount), colorId, &err),  (mode != TestMode::MIXED));
        ...
        for (int i = 0; i < 4; i++)
        {
            keys.assign(1,key[i]);
            s = sign_multisig(a_and_b, keys, txTo[0], 0, mode);
            BOOST_CHECK_MESSAGE(!VerifyScript(s, a_and_b, nullptr, flags, MutableTransactionSignatureChecker(&txTo[0], 0, amount), colorId, &err), strprintf("a&b 1: %d", i));
            BOOST_CHECK_MESSAGE(err == SCRIPT_ERR_INVALID_STACK_OPERATION, ScriptErrorString(err));

            keys.assign(1,key[1]);
            keys.push_back(key[i]);
            s = sign_multisig(a_and_b, keys, txTo[0], 0, mode);
            BOOST_CHECK_MESSAGE(!VerifyScript(s, a_and_b, nullptr, flags, MutableTransactionSignatureChecker(&txTo[0], 0, amount), colorId, &err), strprintf("a&b 2: %d", i));
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            if (i == 0 || i == 1)
            {
                BOOST_CHECK_MESSAGE(VerifyScript(s, a_or_b, nullptr, flags, MutableTransactionSignatureChecker(&txTo[1], 0, amount), colorId, &err), strprintf("a|b: %d", i));
                BOOST_CHECK_MESSAGE(err == SCRIPT_ERR_OK, ScriptErrorString(err));
            }
            else
            {
        ...
        s.clear();
        s << OP_0 << OP_1;
        BOOST_CHECK(!VerifyScript(s, a_or_b, nullptr, flags, MutableTransactionSignatureChecker(&txTo[1], 0, amount), colorId, &err));
        ...
        for (int i = 0; i < 4; i++)
            for (int j = 0; j < 4; j++)
            {
                ...
                if (i < j && i < 3 && j < 3)
                {
                    BOOST_CHECK_MESSAGE(VerifyScript(s, escrow, nullptr, flags, MutableTransactionSignatureChecker(&txTo[2], 0, amount), colorId, &err) == (mode != TestMode::MIXED), strprintf("escrow 1: %d %d", i, j));
                    if(mode == TestMode::MIXED)
                        BOOST_CHECK_MESSAGE(err == SCRIPT_ERR_MIXED_SCHEME_MULTISIG, ScriptErrorString(err));
                    else
                        BOOST_CHECK_MESSAGE(err == SCRIPT_ERR_OK, ScriptErrorString(err));
                ...
                {
                    BOOST_CHECK_MESSAGE(!VerifyScript(s, escrow, nullptr, flags, MutableTransactionSignatureChecker(&txTo[2], 0, amount), colorId, &err), strprintf("escrow 2: %d %d", i, j));
