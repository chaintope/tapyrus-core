...
BOOST_FIXTURE_TEST_CASE(txindex_initial_sync, TestChainSetup)
{
    ...
    for (const auto& txn : m_coinbase_txns) {
        BOOST_CHECK(!txindex.FindTx(txn->GetHashMalFix(), block_hash, tx_disk));
    }
    ...
    for (const auto& txn : m_coinbase_txns) {
        if (!txindex.FindTx(txn->GetHashMalFix(), block_hash, tx_disk)) {
            BOOST_ERROR("FindTx failed");
        } else if (tx_disk->GetHashMalFix() != txn->GetHashMalFix()) {
