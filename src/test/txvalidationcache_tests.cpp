...
BOOST_FIXTURE_TEST_CASE(checkinputs_test, TestChainSetup)
{
    ...
    for(int count : {0, 1}) //loop for Schnorrr and ECDSA signature
    {
        ...
        // for the same tx with a different witness.
        {
            ...
            if(count == 0)
                ProduceSignature(keystore, MutableTransactionSignatureCreator(&valid_with_witness_tx, 0, 11*CENT, SIGHASH_ALL, SignatureScheme::ECDSA), spend_tx.vout[1].scriptPubKey, sigdata);
            else
                ProduceSignature(keystore, MutableTransactionSignatureCreator(&valid_with_witness_tx, 0, 11*CENT, SIGHASH_ALL, SignatureScheme::SCHNORR), spend_tx.vout[1].scriptPubKey, sigdata);
            ...
            BOOST_CHECK(CheckInputs(valid_with_witness_tx, state, pcoinsTip.get(), true, 0, true, true, txdata, inColoredCoinBalances, nullptr));
            BOOST_CHECK(CheckInputs(valid_with_witness_tx, state, pcoinsTip.get(), true, STANDARD_NOT_MANDATORY_VERIFY_FLAGS, true, true, txdata, inColoredCoinBalances, nullptr));
            BOOST_CHECK(CheckInputs(valid_with_witness_tx, state, pcoinsTip.get(), true, STANDARD_SCRIPT_VERIFY_FLAGS, true, true, txdata, inColoredCoinBalances, nullptr));
        }
        {
            ...
            for (int i=0; i<2; ++i) {
                SignatureData sigdata;
                if(count == 0)
                    ProduceSignature(keystore, MutableTransactionSignatureCreator(&tx, i, 11*CENT, SIGHASH_ALL, SignatureScheme::ECDSA), spend_tx.vout[i].scriptPubKey, sigdata);
                else
                    ProduceSignature(keystore, MutableTransactionSignatureCreator(&tx, i, 11*CENT, SIGHASH_ALL, SignatureScheme::SCHNORR), spend_tx.vout[i].scriptPubKey, sigdata);
            ...
            BOOST_CHECK(CheckInputs(tx, state, pcoinsTip.get(), true, 0, true, false, txdata, inColoredCoinBalances, nullptr));
            BOOST_CHECK(CheckInputs(tx, state, pcoinsTip.get(), true, STANDARD_NOT_MANDATORY_VERIFY_FLAGS, true, false, txdata, inColoredCoinBalances, nullptr));
            BOOST_CHECK(CheckInputs(tx, state, pcoinsTip.get(), true, STANDARD_SCRIPT_VERIFY_FLAGS, true, false, txdata, inColoredCoinBalances, nullptr));


            std::vector<CScriptCheck> scriptchecks;
            // Make sure this transaction was not cached (ie because the first
            // input was valid)
            BOOST_CHECK(CheckInputs(tx, state, pcoinsTip.get(), true, 0, true, true, txdata, inColoredCoinBalances, &scriptchecks));
            BOOST_CHECK(CheckInputs(tx, state, pcoinsTip.get(), true, STANDARD_NOT_MANDATORY_VERIFY_FLAGS, true, true, txdata, inColoredCoinBalances, &scriptchecks));
            BOOST_CHECK(CheckInputs(tx, state, pcoinsTip.get(), true, STANDARD_SCRIPT_VERIFY_FLAGS, true, true, txdata, inColoredCoinBalances, &scriptchecks));
