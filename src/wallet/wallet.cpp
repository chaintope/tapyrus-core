...
CAmount CWallet::GetDebit(const CTxIn &txin, const isminefilter& filter, ColorIdentifier& colorId) const
{
    LOCK(cs_wallet);
...
TxColoredCoinBalancesMap CWallet::GetDebit(const CTransaction& tx, const isminefilter& filter) const
{
    ...
    for (const CTxIn& txin : tx.vin) {
        CAmount debit = GetDebit(txin, filter, colorId);
        nDebit[colorId] += debit;

        if (!MoneyRange(nDebit[colorId]))
            throw std::runtime_error(std::string(__func__) + ": value out of range");
    }
    return nDebit;
}
...
CAmount CWalletTx::GetDebit(const isminefilter& filter, const ColorIdentifier& colorId) const
{
    if (tx->vin.empty())
        return 0;
...
CAmount CWalletTx::GetCredit(const isminefilter& filter, const ColorIdentifier& colorId) const
{
    CAmount credit = 0;
    if (filter & ISMINE_SPENDABLE)
    {
    ...
    if (filter & ISMINE_WATCH_ONLY)
    {
        if (fWatchCreditCached)
            credit += nWatchCreditCached[colorId];
        else
        {
...
void CWallet::AvailableCoins(std::vector<COutput> &vCoins, bool fOnlySafe, const CCoinControl *coinControl, const CAmount &nMinimumAmount, const CAmount &nMaximumAmount, const CAmount &nMinimumSumAmount, const uint64_t nMaximumCount, const int nMinDepth, const int nMaxDepth, bool only_token) const
{
    ...
    for (const auto& entry : mapWallet)
    {
        ...
        for (unsigned int i = 0; i < pcoin->tx->vout.size(); i++) {
            ...
            {
                // For REISSUABLE token issuance, only include TPC UTXOs whose script
                // defines this token's color (the anchor UTXO). When fAllowOtherInputs
                // is true, all TPC UTXOs are visible so tx2 can cover fees even if the
                // anchor alone is insufficient.
...
bool CWallet::SelectCoins(const std::vector<COutput>& vAvailableCoins, const CAmount& nTargetValue, const ColorIdentifier& colorId, std::set<CInputCoin>& setCoinsRet, CAmount& nValueRet, const CCoinControl& coin_control, CoinSelectionParams& coin_selection_params, bool& bnb_used) const
{
    std::vector<COutput> vCoins(vAvailableCoins);

    // remove coins that are not mach with the colorId
    for (std::vector<COutput>::iterator it = vCoins.begin(); it != vCoins.end();)
    {
        if (!it->IsColoredWith(colorId))
            it = vCoins.erase(it);
        else
            ++it;
    }
                                ...
                                ChangePosInOut& mapChangePosInOut, std::string& strFailReason, const CCoinControl& coin_control, bool sign)
{
    ...
    // Add initial value for TPC. Even if the value would be sent of TPC is zero, it should be
    // treated in loops below for paying fee.
    mapValue[ColorIdentifier()] = 0;

    ChangePosInOut mapChangePosRequest(mapChangePosInOut);
    if(mapChangePosRequest.empty())
       mapChangePosRequest[ColorIdentifier()] = -1;
    ...
    int nBytes;
    {
        std::map<ColorIdentifier, std::set<CInputCoin>, ColorIdentifierCompare> mapCoins;
        LOCK2(cs_main, cs_wallet);
        {
            ...
            std::map<ColorIdentifier, CAmount, ColorIdentifierCompare> mapValueIn;
            ...
            while (true)
            {
                ...
                for (const auto& i : mapValue) {
                    ...
                    if (nChange > 0)
                    {
                        CScript sc = CScript() << colorId.toVector() << OP_COLOR;
                        sc += scriptChange;

                        // Fill a colored vout to ourself
                ...
                if (nChange > 0)
                {
                    ...
                    {
                        //if changeposition is fixed for this transaction insert it there
                        ...
                        {
                            if ((unsigned int)mapChangePosInOut[colorId] > txNew.vout.size())
                            {
                            ...
                            {
                                if(i.first.type != TokenTypes::NONE
                ...
                //
                for (const auto& i : mapCoins) {
                    for (const auto& coin : i.second) {
...
bool CWallet::Verify(std::string wallet_file, bool salvage_wallet, std::string& error_string, std::string& warning_string)
{
    ...
    for (auto wallet : GetWallets()) {
        if (fs::path(GetWalletDir() / wallet->GetName()) == wallet_path) {
