...
void CTxMemPool::UpdateForDescendants(txiter updateIt, cacheMap &cachedDescendants, const std::set<uint256> &setExclude)
{
    ...
    for (txiter cit : setAllDescendants) {
        if (!setExclude.count(cit->GetTx().GetHashMalFix())) {
...
bool CTxMemPool::CalculateMemPoolAncestors(const CTxMemPoolEntry &entry, setEntries &setAncestors, uint64_t limitAncestorCount, uint64_t limitAncestorSize, uint64_t limitDescendantCount, uint64_t limitDescendantSize, std::string &errString, bool fSearchForParents /* = true */) const
{
    ...
    while (!parentHashes.empty()) {
        ...
        if (static_cast<uint64_t>(stageit->GetSizeWithDescendants() + entry.GetTxSize()) > limitDescendantSize) {
            ...
            return false;
        } else if (static_cast<uint64_t>(totalSizeWithAncestors) > limitAncestorSize) {
...
void CTxMemPool::addUnchecked(const uint256& hash, const CTxMemPoolEntry &entry, setEntries &setAncestors, bool validFeeEstimate)
{
    ...
    for (unsigned int i = 0; i < tx.vin.size(); i++) {
        ...
        setParentTransactions.insert(tx.vin[i].prevout.hashMalFix);
    }
...
void CTxMemPool::removeRecursive(const CTransaction &origTx, MemPoolRemovalReason reason)
{
    // Remove transaction from memory pool
    {
        ...
        txiter origit = mapTx.find(origTx.GetHashMalFix());
        if (origit != mapTx.end()) {
            ...
            for (unsigned int i = 0; i < origTx.vout.size(); i++) {
                auto it = mapNextTx.find(COutPoint(origTx.GetHashMalFix(), i));
                if (it == mapNextTx.end())
                    continue;
                txiter nextit = mapTx.find(it->second->GetHashMalFix());
...
void CTxMemPool::removeConflicts(const CTransaction &tx)
{
    ...
    for (const CTxIn &txin : tx.vin) {
        ...
        if (it != mapNextTx.end()) {
            ...
            if (txConflict != tx)
            {
                ClearPrioritisation(txConflict.GetHashMalFix());
...
void CTxMemPool::removeForBlock(const std::vector<CTransactionRef>& vtx, unsigned int nBlockHeight)
{
    ...
    for (const auto& tx : vtx)
    {
        txiter it = mapTx.find(tx->GetHashMalFix());
        if (it != mapTx.end()) {
            setEntries stage;
            stage.insert(it);
        ...
        removeConflicts(*tx);
        ClearPrioritisation(tx->GetHashMalFix());
    }
...
void CTxMemPool::removeForScriptFlagChange(unsigned int newMempoolScriptFlags, int32_t newBlockHeight)
{
    ...
    for (indexed_transaction_set::const_iterator it = mapTx.begin(); it != mapTx.end(); ++it) {
        ...
        if (!CheckInputs(tx, state, view, true, newMempoolScriptFlags, false, false)) {
            txToRemove.insert(it);
            continue;
        }
...
void CTxMemPool::check(const CCoinsViewCache *pcoins) const
{
    ...
    for (indexed_transaction_set::const_iterator it = mapTx.begin(); it != mapTx.end(); it++) {
        ...
        assert(setParentCheck.size() == GetMemPoolParents(it).size());
    ...
    for (auto it = mapNextTx.cbegin(); it != mapNextTx.cend(); it++) {
        uint256 hash = it->second->GetHashMalFix();
...
void CTxMemPool::queryHashes(std::vector<uint256>& vtxid)
{
    ...
    for (auto it : iters) {
        vtxid.push_back(it->GetTx().GetHashMalFix());
    }
}
...
{
    for (unsigned int i = 0; i < tx.vin.size(); i++)
        if (exists(tx.vin[i].prevout.hashMalFix))
            return false;
    return true;
}
    ...
    while (!mapTx.empty() && DynamicMemoryUsage() > sizelimit) {
        ...
        if (pvNoSpendsRemaining) {
            for (const CTransaction& tx : txn) {
                for (const CTxIn& txin : tx.vin) {
                    if (exists(txin.prevout.hashMalFix)) continue;
