...
static bool AppInit(int argc, char* argv[])
{
    ...
    // Parameters
    //
    // If Qt is used, parameters/tapyrus.conf are parsed in qt/tapyrus.cpp's main()
    std::string error;
    SetupServerArgs();
    ...
    if (HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        {
            strUsage += "\nUsage:  tapyrusd [options]                     Start " PACKAGE_NAME " Daemon\n";
    ...
    try
    {
        ...
        for (int i = 1; i < argc; i++) {
            if (!IsSwitchChar(argv[i][0])) {
                fprintf(stderr, "Error: Command line contains unexpected token '%s', see tapyrusd -h for a list of options.\n", argv[i]);
                return false;
            }
        }

        // -server defaults to true for tapyrusd but not for the GUI so do this here
...
#endif
            fprintf(stdout, "Tapyrus server starting\n");
...
int main(int argc, char* argv[])
{
    ...
    // Connect tapyrusd signal handlers
    noui_connect();
