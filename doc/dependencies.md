...
`Minimum tested` in the table below is the oldest version of each package this repo actively builds and tests against, not necessarily the absolute floor `CMakeLists.txt`'s `find_package(... <version> ...)` would technically accept. For most packages there's only ever been one tested version, so `Minimum tested` just repeats the pinned `Version` column -- there's no independently-enforced floor to report. Boost, Expat, FreeType, libevent, and systemtap are the exception: each was bumped to a newer `Version` in the same change that added them here, and `Minimum tested` is pinned to the version they were bumped *from* -- the weekly `minimum-dependency-versions` CI job (`.github/workflows/weekly-heavy-tests.yml`) actually swaps `depends/packages/*.mk` to that exact version and does a full from-scratch build to verify it. This is deliberately narrower than what `find_package()` itself permits: `cmake/module/AddBoostIfNeeded.cmake:85` still says `find_package(Boost 1.73.0 REQUIRED)` and `CMakeLists.txt:366` still says `find_package(Libevent 2.1.8 MODULE REQUIRED)`, both older than the `Minimum tested` values below -- those C++-side floors are untouched and still technically satisfiable by older versions, they're just not the ones CI actively proves still work. fontconfig, libXau, and libxkbcommon were bumped in the same change too, but aren't given a tested floor below the current `Version`: their older releases used a different archive format or download host than the currently-pinned `.mk` recipe (fontconfig/libXau: `.tar.bz2` instead of today's `.tar.xz`/`.tar.gz`; libxkbcommon: `xkbcommon.org` instead of today's GitHub-tags fetch), which would need real per-field swap logic in the minimum-version CI job rather than a plain version+hash substitution -- judged not worth the added complexity for now.
