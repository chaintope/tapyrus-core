...
"""Example logging Tapyrus Core utxo set cache flushes utilizing
    the utxocache:flush tracepoint."""

# USAGE:  ./contrib/tracing/log_utxocache_flush.py path/to/tapyrusd
...
def main(tapyrusd_path):
    tapyrusd_with_usdts = USDT(path=str(tapyrusd_path))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    tapyrusd_with_usdts.enable_probe(
        probe="utxocache_flush", fn_name="trace_flush")
    b = BPF(text=program, usdt_contexts=[tapyrusd_with_usdts])
