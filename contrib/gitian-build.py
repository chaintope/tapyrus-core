...
# Copyright (c) 2021 The Tapyrus Core developers
...
def build():
    ...
    os.makedirs('tapyrus-core-binaries/' + args.version, exist_ok=True)
    ...
    subprocess.check_call(['make', '-C', '../tapyrus-core/depends', 'download', 'SOURCES_PATH=' + os.getcwd() + '/cache/common'])
...
def main():
    ...
    parser.add_argument('-u', '--url', dest='url', default='https://github.com/chaintope/tapyrus-core', help='Specify the URL of the repository. Default is %(default)s')
    ...
    if args.macos and not os.path.isfile('gitian-builder/inputs/MacOSX13.3.sdk.tar.gz'):
    ...
    if args.pull:
        ...
        os.chdir('../gitian-builder/inputs/tapyrus-core')
        subprocess.check_call(['git', 'fetch', args.url, 'refs/pull/'+args.version+'/merge'])
