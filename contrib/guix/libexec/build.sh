...
(
    ...
    # Build Tapyrus Core
    cmake --build build --parallel -j"$JOBS" ${V:+--verbose} --target tapyrusd --target tapyrus-cli --target tapyrus-genesis --target tapyrus-tx

    # Check that symbol/security checks tools are sane.
    cmake --build build --target test-security-check ${V:+--verbose}
    # Perform basic security checks on a series of executables.
    cmake --build build --target check-security ${V:+--verbose}
    # Check that executables only contain allowed version symbols.
    cmake --build build --target check-symbols ${V:+--verbose}
