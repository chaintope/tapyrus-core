...
# shellcheck source=contrib/guix/libexec/realpath.bash
source contrib/guix/libexec/realpath.bash

# shellcheck source=contrib/guix/libexec/git-utils.bash
source contrib/guix/libexec/git-utils.bash
...
# Setup getent compatibility for macOS
# On macOS, getent is not available by default, but we only need basic services lookup
if ! command -v getent > /dev/null 2>&1; then
    echo "INFO: getent not found, using compatibility wrapper for macOS"
    
    # Create a getent function that covers our specific use case
    getent() {
        "$(dirname "${BASH_SOURCE[0]}")/getent-compat.sh" "$@"
    }
    export -f getent
fi
...
DISTNAME="${DISTNAME:-tapyrus-${VERSION}}"
