# Bootstrappable Tapyrus Core Builds

This directory contains the files necessary to perform bootstrappable Tapyrus
Core builds.
...
To build Tapyrus Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Tapyrus Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/tapyrus-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository, Guix build outputs can be attested to in a similar repository
structure for Tapyrus Core.
