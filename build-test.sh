...
RETRY apt-get install --no-install-recommends --no-upgrade -qq $PACKAGES $BUILD_PACKAGES
...
if [ "$RUN_BENCH" = "true" ]; then LD_LIBRARY_PATH=${BITBUCKET_CLONE_DIR}/depends/$HOST/lib $OUTDIR/bin/bench_bitcoin -scaling=0.001 ; fi
