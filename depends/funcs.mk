...
$($(1)_staged): | $($(1)_built)
	...
	if [ -d "$($(1)_extract_dir)" ] && [ "$($(1)_extract_dir)" != "." ] && [ "$($(1)_extract_dir)" != ".." ]; then rm -rf $($(1)_extract_dir); fi
	touch $$@
...
$($(1)_cached): | $($(1)_dependencies) $($(1)_postprocessed)
	...
	mkdir -p $$(@D)
	if [ -d "$$(@D)" ] && [ "$$(@D)" != "." ] && [ "$$(@D)" != ".." ]; then rm -rf $$(@D); fi && mkdir -p $$(@D)
	mv $$($(1)_staging_dir)/$$(@F) $$(@)
	if [ -d "$($(1)_staging_dir)" ] && [ "$($(1)_staging_dir)" != "." ] && [ "$($(1)_staging_dir)" != ".." ]; then rm -rf $($(1)_staging_dir); fi
