...
class PruneTest(BitcoinTestFramework):
    def set_test_params(self):
        ...
        self.full_node_default_args = ["-dev", "-maxreceivebuffer=20000", "-checkblocks=5", "-limitdescendantcount=100", "-limitdescendantsize=5000", "-limitancestorcount=100", "-limitancestorsize=5000" ]
        # Create nodes 3 and 4 to test manual pruning (they will be re-started with manual pruning later)
        # Create nodes 5 to test wallet in prune mode, but do not connect
        self.extra_args = [self.full_node_default_args,
                                    self.full_node_default_args,
                                    ["-dev", "-maxreceivebuffer=20000", "-prune=550"],
                                    ["-dev", "-maxreceivebuffer=20000"],
                                    ["-dev", "-maxreceivebuffer=20000"],
                                    ["-dev", "-prune=550"]]
    ...
    def create_big_chain(self):
        ...
        self.nodes[1].generate(200, self.signblockprivkey_wif)
    ...
    def manual_test(self, node_number, use_timestamp):
        ...
        node.generate(288, self.signblockprivkey_wif)
        prune(1000)
        if has_block(2):
            raise AssertionError("blk00002.dat is still there, should be pruned by now")
        if has_block(3):
