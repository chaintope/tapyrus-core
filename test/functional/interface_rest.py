...
class RESTTest (BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        assert_greater_than(int(response.getheader('content-length')), 105)
        response_bytes = response.read()

        # Compare with block header
        response_header = self.test_rest_request("/headers/1/{}".format(bb_hash), req_type=ReqType.BIN, ret_type=RetType.OBJ)
        assert_equal(int(response_header.getheader('content-length')), 4+32+32+32+1+65+4)
        response_header_bytes = response_header.read()
        assert_equal(response_bytes[:4+32+32+32+1+65+4], response_header_bytes)
        ...
        for key in ['hash', 'confirmations', 'height', 'features', 'merkleroot', 'time', 'previousblockhash']:
