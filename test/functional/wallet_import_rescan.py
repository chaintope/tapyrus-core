...
import RPC call and sends TPC to it. Then other nodes import the addresses,
and the test makes listtransactions and getbalance calls to confirm that the
importing node either did or did not execute rescans picking up the send
transactions.

In the second part of the test, node 0 sends more TPC to each address, and the
...
class Variant(collections.namedtuple("Variant", "call data rescan prune")):
    ...
    def check(self, txid=None, amount=None, confirmations=None):
        ...
        addresses = self.node.listreceivedbyaddress(include_watchonly=True, address_filter=self.address['address'])
