...
class MempoolCoinbaseTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        res = create_colored_transaction(1, 1000, self.nodes[0])
        colorid = res['color']
        ctxids = res['txids']

        # Mine four blocks. After this, nodes[0] blocks
        # 101, 102, and 103 are spend-able.
        new_blocks = self.nodes[1].generate(4, self.signblockprivkey_wif)
        self.sync_all()
        ...
        for (address_n0, address_n1, height, amt) in  [(node0_address, node1_address, 0, 49.99),(node0_caddress, node1_caddress, 5, 500)]:
            ...
            b = [ self.nodes[0].getblockhash(n) for n in range(height + 1, height + 5) ]
            ...
            if amt == 500:
                vout = next(i for i, out in enumerate(self.nodes[0].getrawtransaction(ctxids[1], 1)["vout"]) if out["value"] == 1000)
                timelock_tx = self.nodes[0].createrawtransaction([{"txid": ctxids[1], "vout": vout}], {address_n0: amt})
            else:
            ...
            assert_equal(set(self.nodes[0].getrawmempool()), {spend_101_id, spend_102_id, spend_102_1_id, spend_103_id, spend_103_1_id, ctxids[0], ctxids[1]})
